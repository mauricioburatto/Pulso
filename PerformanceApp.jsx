...
// Extrai um array JSON de um texto, reparando cortes por limite de tokens
// (fecha o último objeto completo e descarta o restante incompleto).
function extractJsonArray(text) {
  ...
  try {
    ...
    if (lastGoodEnd === -1) throw new Error("A resposta veio incompleta e não foi possível recuperar nenhum item.");
    const repaired = candidate.slice(0, lastGoodEnd + 1) + "]";
    return JSON.parse(repaired);
  }
}

// Extrai um objeto JSON de um texto, tolerando texto extra antes/depois.
...
function hashPassword(pw) {
  let h1 = 0xdeadbeef ^ pw.length;
  let h2 = 0x41c6ce57 ^ pw.length;
...
function ProfileGate({ onEnter }) {
  ...
  useEffect(() => {
    storeGet("accounts", []).then((list) => {
      setAccounts(list);
      setMode(list.length > 0 ? "login" : "signup");
    });
  }, []);
  ...
  async function handleSignup() {
    ...
    if ((accounts || []).some((a) => a.email === emailNorm)) {
      setError("Já existe uma conta com esse email. Faça login.");
      return;
    }
    ...
    setAccounts(next);
    await storeSet("accounts", next);
    await storeSet(`athlete:${id}:core`, emptyCore());
    await storeSet(`athlete:${id}:photos`, []);
    onEnter(account);
  }
  ...
  async function handleLogin() {
    ...
    if (!account || account.passwordHash !== hashPassword(loginPassword)) {
  ...
  async function handleSendCode() {
    ...
    const exists = (accounts || []).some((a) => a.email === emailNorm);
    if (!exists) {
      setError("Não encontramos nenhuma conta com esse email neste dispositivo.");
      return;
    }
  ...
  async function handleVerifyAndReset() {
    ...
    if (resetCodeInput.trim() !== resetCode) {
    ...
    const emailNorm = resetEmail.trim().toLowerCase();
    const list = accounts || [];
    const idx = list.findIndex((a) => a.email === emailNorm);
    if (idx === -1) {
      setError("Não encontramos nenhuma conta com esse email neste dispositivo.");
      return;
    }
    const next = list.map((a, i) => (i === idx ? { ...a, passwordHash: hashPassword(resetPassword) } : a));
    setAccounts(next);
    await storeSet("accounts", next);
  ...
  if (accounts === null) {
    return (
      <div style={{ color: T.textMuted, fontFamily: "Inter", padding: 40 }}>
        Carregando...
      </div>
    );
  }
          ...
          <Card>
            ...
            {resetSuccess ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                ...
                <Btn variant="primary" onClick={handleSendCode}>
                  Enviar código
                </Btn>
                <button
                  onClick={() => { setMode("login"); resetFlowReset(); }}
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                ...
                >
                  Este app ainda não tem servidor de email para enviar o código de verdade — então,
                  só pra você conseguir testar o fluxo completo, o código está aparecendo aqui
                  embaixo em vez de chegar na caixa de entrada de <strong style={{ color: T.textPrimary }}>{resetEmail}</strong>.
                  <div
                    style={{
                      fontFamily: "JetBrains Mono",
                      fontSize: 26,
                ...
                <Btn variant="primary" onClick={handleVerifyAndReset}>
                  Confirmar código e salvar senha
                </Btn>
...
function TrainingItemDetail({ item, core, updateCore, profile, nextGoal, onToggle }) {
  ...
  async function generateDetail() {
    ...
    try {
      ...
      const detail = extractJsonObject(text);
      updateCore({
        ...core,
...
function TreinoAtualImport({ core, updateCore, profile }) {
  ...
  async function analyze() {
    ...
    try {
      ...
      const entry = { id: "ta_" + Date.now(), date: new Date().toISOString(), sourceText: description.trim(), ...result };
...
function Treinos({ core, updateCore, profile }) {
  ...
  async function generatePlan(toGoal = false) {
    ...
    try {
      ...
      const weeksToGoal =
        toGoal && nextGoal ? Math.max(1, Math.ceil(daysUntil(nextGoal.targetDate) / 7)) : null;
      // Gera no máximo 2 semanas por vez para caber com folga no limite de resposta da IA.
      const cappedWeeks = weeksToGoal ? Math.min(weeksToGoal, 2) : Math.min(weeks, 2);
...
${others.length ? `Também pratica em paralelo: ${others.map((m) => `${m.name} (${m.frequency})`).join(", ")}.` : "Sem outras modalidades cadastradas."}
...
${preferences.trim() ? `Preferências/restrições do atleta para esta planilha: ${preferences.trim()}.` : ""}
...
Gere ${cappedWeeks} semana(s) a partir de ${new Date().toISOString().slice(0, 10)}.${
      ...
      const withIds = parsed.map((w) => ({
        id: "pw_" + Math.random().toString(36).slice(2),
        date: w.date,
      ...
      updateCore({ ...core, plannedWorkouts: [...withIds, ...core.plannedWorkouts] });
      if (truncated) {
  ...
  async function generateDetails(item) {
    setDetailLoading((prev) => ({ ...prev, [item.id]: true }));
    setDetailError((prev) => ({ ...prev, [item.id]: "" }));
    try {
      const system = `Você é um treinador esportivo experiente montando a ficha detalhada de UM treino específico, digna de uma planilha profissional de atleta. Responda APENAS com um objeto JSON válido, sem texto antes ou depois, sem markdown, no formato: {"aquecimento":"...","parte_principal":"...","alongamento":"...","volta_a_calma":"...","tempo_estimado_min":numero,"dicas":"..."}. Adapte o conteúdo ao tipo de treino: para corrida/ciclismo/natação use pace (min/km ou min/100m), distância e tempo por trecho; para força/crossfit use séries, repetições, carga (%1RM ou RPE) e descanso entre séries; para lutas use rounds, duração e foco técnico. Seja específico com números sempre que fizer sentido. "parte_principal" pode ter quebras de linha para organizar blocos/séries. Escreva em português do Brasil, claro e pronto para o atleta seguir sem perguntas.`;

      const userMsg = `Atleta: ${profile.name}, nível ${profile.level}.
Treino: ${item.modality || item.type} — ${item.title}
Resumo: ${item.description}
Intensidade: ${item.intensity || "moderado"}
Data: ${item.date}
${nextGoal ? `Meta/prova relacionada: ${nextGoal.title} em ${nextGoal.targetDate}${nextGoal.targetMetric ? `, alvo ${nextGoal.targetMetric}` : ""}.` : ""}`;
      ...
      updateCore({
        ...core,
        plannedWorkouts: core.plannedWorkouts.map((w) => (w.id === item.id ? { ...w, details } : w)),
      });
    } catch (e) {
      console.error(e);
      setDetailError((prev) => ({
        ...prev,
        [item.id]: `Não consegui gerar os detalhes agora${e && e.message ? ` (${e.message})` : ""}. Tente de novo.`,
      }));
    } finally {
      setDetailLoading((prev) => ({ ...prev, [item.id]: false }));
    }
  }
        ...
        messages: [
          {
            ...
            content: [
              ...
              { type: "image", source: { type: "base64", media_type: "image/jpeg", data: sideBase64 } },
            ],
          },
        ],
        maxTokens: 1000,
...
function Evolucao({ athleteId, profile, core, updateCore }) {
  ...
  useEffect(() => {
    storeGet(`athlete:${athleteId}:photos`, []).then(setPhotos);
  }, [athleteId]);
  ...
  async function savePhoto() {
    ...
    const entry = { id: "ph_" + Date.now(), date, image: pendingPreview, notes: notes.trim() };
    const next = [entry, ...(photos || [])];
    setPhotos(next);
    await storeSet(`athlete:${athleteId}:photos`, next);
    setPendingFile(null);
              ...
              <div style={{ display: "flex", gap: 14, alignItems: "flex-start", flexWrap: "wrap" }}>
                ...
                <div style={{ flex: 1, minWidth: 220 }}>
                  ...
                  <div style={{ marginTop: 8 }}>
                    <Btn variant="primary" onClick={savePhoto}>
                      Salvar foto
                    </Btn>
                  </div>
                </div>
  ...
  async function generateWithAI() {
    ...
    try {
      ...
      const questionnaireLines = [
        q.rotina.trim() ? `Rotina diária: ${q.rotina.trim()}` : "",
        q.alimentacaoAtual.trim() ? `Alimentação atual: ${q.alimentacaoAtual.trim()}` : "",
        q.gosta.trim() ? `Gosta de comer: ${q.gosta.trim()}` : "",
        q.naoGosta.trim() ? `Não gosta / não come: ${q.naoGosta.trim()}` : "",
        q.paladar.trim() ? `Paladar/preferências: ${q.paladar.trim()}` : "",
        q.suplementos.trim() ? `Suplementação em uso: ${q.suplementos.trim()}` : "",
        q.observacoes.trim() ? `Considerações adicionais do atleta: ${q.observacoes.trim()}` : "",
      ]
        .filter(Boolean)
        .join("\n");
...
Histórico de treinos (mais recente primeiro): ${JSON.stringify(recent)}`;
      ...
      const entry = { id: "an_" + Date.now(), date: new Date().toISOString(), content: text };
...
Suplementação programada: ${JSON.stringify(supplementsToday.map((s) => s.name))}`;
      ...
      const entry = { id: "rp_" + Date.now(), date: new Date().toISOString(), type: kind, content: text };
...
export default function App() {
  ...
  // Ao carregar, verifica se há uma sessão salva (login automático)
  useEffect(() => {
    (async () => {
      const sessionId = await storeGet("session", null);
      if (sessionId) {
        const accounts = await storeGet("accounts", []);
  ...
  useEffect(() => {
    if (!profile) return;
    storeGet(`athlete:${profile.id}:core`, emptyCore()).then(setCore);
  }, [profile]);

  const updateCore = useCallback(
    (next) => {
      setCore(next);
      if (profile) storeSet(`athlete:${profile.id}:core`, next);
    },
    [profile]
  );
